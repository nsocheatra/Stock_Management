...
let dbInstance: DbWrapper | undefined;
...
class DbWrapper {
  ...
  private async fallbackToSqlite() {
    ...
    console.log(`[db] PostgreSQL unavailable, using SQLite (${dbPath})`);
  ...
  private async initSchema(backend: Backend) {
    ...
    const upsertSetting = async (key: string, value: string) => {
      if (this.sqlite) {
        this.getClient().prepare("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)").run(key, value);
      } else {
        await this.getClient().query("INSERT INTO settings (key, value) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value", [key, value]);
      }
    };

    upsertSetting("printer_type", "browser");
    upsertSetting("paper_width", "80");
    upsertSetting("receipt_header", "=== MY STORE ===");
    upsertSetting("receipt_footer", "Thank you! Visit again.");
    upsertSetting("receipt_copies", "1");
    upsertSetting("auto_print", "0");
    upsertSetting("store_name", "My Store");
    upsertSetting("store_address", "");
    upsertSetting("store_phone", "");
    upsertSetting("printer_ip", "");
    upsertSetting("printer_port", "9100");
    upsertSetting("telegram_notify_low_stock", "1");
    upsertSetting("telegram_notify_daily", "1");
    upsertSetting("payment_default_method", "cash");
    upsertSetting("payment_methods_enabled", "cash,bank_transfer");
    upsertSetting("tax_rate", "0");
    upsertSetting("tax_label", "Tax");
    ...
    const userCount = await (async () => {
      if (this.sqlite) {
        const r = this.getClient().prepare(countSql).get() as { count: number };
        return r.count;
      }
      const r = await this.getClient().query(countSql);
    ...
    if (userCount === 0) {
      if (adminEmail && adminPassword && adminPin) {
        ...
        if (this.sqlite) {
          this.getClient().prepare("INSERT INTO users (name, email, password_hash, role, pin) VALUES (?, ?, ?, ?, ?)").run("Administrator", adminEmail, password_hash, "admin", adminPin);
        } else {
          await this.getClient().query("INSERT INTO users (name, email, password_hash, role, pin) VALUES ($1, $2, $3, $4, $5)", ["Administrator", adminEmail, password_hash, "admin", adminPin]);
        }
      }
    } else if (adminEmail && adminPassword && adminPin) {
      const password_hash = hashPassword(adminPassword);
      if (this.sqlite) {
        this.getClient().prepare("UPDATE users SET password_hash = ?, pin = ? WHERE email = ? AND (password_hash = '' OR password_hash IS NULL)").run(password_hash, adminPin, adminEmail);
      } else {
        await this.getClient().query("UPDATE users SET password_hash = $1, pin = $2 WHERE email = $3 AND (password_hash = '' OR password_hash IS NULL)", [password_hash, adminPin, adminEmail]);
      }
    }
  }
}
