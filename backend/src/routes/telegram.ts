...
router.post("/notify", requireAuth, async (req: Request, res: Response) => {
  const { message } = req.body;
  const token = await getSecret("telegram_bot_token");
  const chatIdsRaw = await getSecret("telegram_chat_ids");
  const enabled = (await getSecret("telegram_enabled")) === "1";
  if (!token || !chatIdsRaw || !enabled) { res.json({ success: true }); return; }
...
router.post("/webhook", async (req: Request, res: Response) => {
  try {
    ...
    if (!message) { res.json({ ok: true }); return; }
