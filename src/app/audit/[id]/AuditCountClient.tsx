      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex gap-2">
          {auditStatus === "in_progress" && allCounted && (
            <form action={async (fd: FormData) => { fd.set("audit_id", String(auditId)); await completeAudit(fd); router.refresh(); }}>
              <button
                type="submit"
          ...
          {auditStatus === "completed" && discrepancies.length > 0 && (
            <form action={async (fd: FormData) => { fd.set("audit_id", String(auditId)); await applyAuditCorrections(fd); router.refresh(); }}>
              <button
                type="submit"
