...
import {
  ...
  FileText, Calendar, Plus, ChevronDown, Edit3, Save,
} from "lucide-react";
import {
...
export default function BroadcastsClient({ broadcasts, templates }: { broadcasts: Broadcast[]; templates: Template[] }) {
  ...
  const saveEdit = async () => {
    ...
    fd.set("name", editName.trim() || "Untitled");
    ...
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
      ...
      <div className="lg:col-span-2 bg-surface-blur border-surface rounded-2xl p-5 shadow-xl space-y-4">
        ...
        <form onSubmit={handleCreate} className="space-y-3">
          <div>
            ...
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Summer Sale Announcement" className="input-field" />
          </div>
          <div>
            ...
            <textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Type your broadcast message..." rows={5} className="input-field resize-none" required />
          </div>
          ...
          <div className="bg-black/20 rounded-xl p-3 border border-surface">
            <p className="text-xs text-faint">
              📊 Will be sent to <strong className="text-default">{broadcasts.length > 0 ? `${broadcasts[0].recipient_count || "all"} conversations` : "all Messenger conversations"}</strong>
            </p>
          </div>
        ...
        <details className="text-xs">
          <summary className="text-violet-400 hover:text-violet-300 cursor-pointer font-medium">Save as template</summary>
          <form onSubmit={saveTemplate} className="mt-2 space-y-2 p-3 rounded-xl bg-black/20 border border-surface">
            <input value={tmplName} onChange={(e) => setTmplName(e.target.value)} placeholder="Template name" className="input-field text-xs" required />
            <textarea value={tmplMsg} onChange={(e) => setTmplMsg(e.target.value)} placeholder="Template message..." rows={2} className="input-field resize-none text-xs" required />
            <button type="submit" className="w-full py-1.5 rounded-lg text-xs font-medium bg-violet-600/30 text-violet-300 hover:bg-violet-600/40 transition-colors cursor-pointer">
              <Plus className="size-3 inline mr-1" /> Save Template
            </button>
          </form>
        </details>
      </div>
      ...
      <div className="lg:col-span-3 bg-surface-blur border-surface rounded-2xl p-5 shadow-xl space-y-3">
        <h3 className="text-sm font-semibold text-default flex items-center justify-between">
          <span>Campaigns ({broadcasts.length})</span>
          {templates.length > 0 && (
            <span className="text-[10px] text-faint">{templates.length} templates</span>
          )}
        </h3>
        {broadcasts.length > 0 ? (
                ...
                <div key={b.id} className="p-3 rounded-xl border border-surface space-y-2">
                  {editingId === b.id ? (
                    <div className="space-y-2">
                      <input value={editName} onChange={(e) => setEditName(e.target.value)} className="input-field text-xs" placeholder="Campaign name" />
                      <textarea value={editMessage} onChange={(e) => setEditMessage(e.target.value)} rows={3} className="input-field resize-none text-xs" />
                      <div className="flex gap-2">
                        <button onClick={saveEdit} className="px-3 py-1.5 rounded-lg text-xs font-medium bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30 cursor-pointer"><CheckCircle className="size-3 inline mr-1" />Save</button>
                        <button onClick={() => setEditingId(null)} className="px-3 py-1.5 rounded-lg text-xs font-medium bg-surface text-muted hover:text-default cursor-pointer">Cancel</button>
                      </div>
                    </div>
                  ) : (
                    <>
                      ...
                      <div className="flex items-center justify-between text-[10px] text-faint">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`capitalize font-medium ${statusColor[b.status] || "text-muted"}`}>{b.status}</span>
                          {b.status === "sent" && <span>{b.sent_count}/{b.recipient_count} delivered</span>}
                        </div>
                        <div className="flex items-center gap-2">
