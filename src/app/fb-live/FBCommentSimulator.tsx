...
export default function FBCommentSimulator({ keywords }: { keywords: string[] }) {
  ...
  const handleSimulate = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      setResult({ text: t("fbLive.main.simulate.noMatch"), ok: false });
    } else {
      setResult({ text: res.error || t("fbLive.main.simulate.failed"), ok: false });
    }
    setComment("");
    router.refresh();
  ...
  return (
    <form onSubmit={handleSimulate} className="space-y-3">
      <div>
        <label className="input-label">{t("fbLive.main.simulate.customerName")}</label>
        <input
          value={customer}
          onChange={(e) => setCustomer(e.target.value)}
          placeholder={t("fbLive.main.simulate.customerPlaceholder")}
          className="input-field"
        />
      </div>
      <div>
        <label className="input-label">{t("fbLive.main.simulate.commentText")}</label>
        <textarea
          value={comment}
          onChange={(e) => setComment(e.target.value)}
          placeholder={t("fbLive.main.simulate.commentPlaceholder")}
      ...
      >
        ...
        {t("fbLive.main.simulate.simulateBtn")}
      </button>
      {result && (
      ...
      {keywords.length > 0 && (
        <div className="text-xs text-faint">
          {t("fbLive.main.simulate.activeKeywords")} {keywords.map((k) => <code key={k} className="text-violet-400 mx-0.5">#{k}</code>)}
        </div>
      )}
    </form>
  );
