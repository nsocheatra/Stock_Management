  ...
  try {
    ...
    if (cmd === "/start") {
      ...
      if (items.length === 0) {
        ...
        reply = "⚠️ <b>Low Stock Products:</b>\n\n" + items.map((p) => `• ${p.name} (${p.sku}): ${p.quantity} / min ${p.min_stock}`).join("\n");
      }
    } else if (cmd === "/summary") {
      const stats = db.prepare("SELECT COUNT(*) as total, COALESCE(SUM(quantity), 0) as total_qty FROM stock_movements WHERE type='OUT' AND date(created_at) = date('now')").get() as { total: number; total_qty: number };
      const revenue = db.prepare("SELECT COALESCE(SUM(sm.quantity * p.price), 0) as total FROM stock_movements sm JOIN products p ON p.id = sm.product_id WHERE sm.type='OUT' AND date(sm.created_at) = date('now')").get() as { total: number };
      const totalProducts = (db.prepare("SELECT COUNT(*) as count FROM products WHERE quantity > 0").get() as { count: number }).count;
      reply = `📊 <b>Daily Summary</b>\n\nSold items: ${stats.total_qty}\nTransactions: ${stats.total}\nRevenue: $${revenue.total.toFixed(2)}\nActive products: ${totalProducts}`;
    } else if (cmd.startsWith("/stock ")) {
      const search = cmd.replace("/stock ", "").trim();
      const products = db.prepare("SELECT name, sku, price, quantity FROM products WHERE LOWER(name) LIKE ? LIMIT 5").all(`%${search}%`) as Array<{ name: string; sku: string; price: number; quantity: number }>;
      if (products.length === 0) {
        reply = `No products found matching "${search}".`;
      } else {
        reply = "📦 <b>Products found:</b>\n\n" + products.map((p) => `• ${p.name} (${p.sku}): $${p.price} — ${p.quantity} in stock`).join("\n");
      }
    } else {
